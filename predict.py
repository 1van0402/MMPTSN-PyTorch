"""Full-scene prediction and GeoTIFF export utilities for the trained classification model."""
...
def load_full_image_time_series_data(file_path):
    """Load full-scene time-series features and split them into spectral, spatial, SAR, and phenological inputs."""
...
def normalize(X, mean=None, std=None):
    """Standardize input features using the provided mean and standard deviation, or compute them from the data."""
...
def classify_samples_in_batches(model, spectral_data, spatial_data, sar_data, phe_data, batch_size=1024):
    """Run batch-wise inference on full-scene samples and return predicted class labels."""
...
def save_classification_as_geotiff(geo_info, classification_result, output_path, resolution=0.0001):
    """Interpolate point-wise classification results to a regular grid and save them as a GeoTIFF file."""
