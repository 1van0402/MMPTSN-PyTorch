"""Multi-Modal Phenological Temporal-Spatial Texture Network (MMPTSN)."""
...
class ChannelAttention(nn.Module):
    """Channel attention module used to reweight feature channels in one-dimensional feature maps."""
    ...
    def forward(self, x):
        """Compute channel-wise attention weights from average- and max-pooled features."""
...
class SpatialAttention(nn.Module):
    """Spatial attention module used to emphasize informative temporal positions in feature maps."""
    ...
    def forward(self, x):
        """Compute spatial attention weights from channel-wise average and maximum responses."""
...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        """Apply channel attention followed by spatial attention to the input features."""
...
class Channel(nn.Module):
    """Fully connected channel attention module for vector features."""
    ...
    def forward(self, x):
        """Generate channel-wise weights for vector features."""

        attn = self.fc(x)
        return self.sigmoid(attn)

class Spatial(nn.Module):
    """Fully connected spatial attention module for vector features."""
    ...
    def forward(self, x):
        """Generate spatial attention weights for vector features."""

        attn = self.fc(x)
        return self.sigmoid(attn)

class CBAM1(nn.Module):
    """Alternative CBAM implementation for one-dimensional feature maps."""
...
class Attention(nn.Module):
    """Temporal attention pooling layer for sequence feature aggregation."""
    ...
    def forward(self, x):
        """Aggregate sequence features using learned temporal attention weights."""
...
class SpectralBranch(nn.Module):
    """Encode monthly spectral features using convolution, BiLSTM, CBAM, and temporal attention."""
    ...
    def __init__(self, input_dim=12, hidden_dims=256, num_layers=2, bidirectional=True, dropout=0.5):
        ...
        self.bn = nn.BatchNorm1d(hidden_dims)  # Batch normalization layer
        self.relu = nn.ReLU(inplace=False)  # Nonlinear activation layer
        ...
        self.cbam = CBAM(lstm_output_dim)  # CBAM attention module
        ...
        self.project = nn.Linear(hidden_dims * 2, 128)  # Project features to a 128-dimensional representation
...
class SpatialBranch(nn.Module):
    """Encode spatial texture features using one-dimensional convolutions and CBAM."""
...
class SARBranch(nn.Module):
    """Encode monthly SAR features using convolution, BiLSTM, CBAM, and temporal attention."""
...
class phenology_branch(nn.Module):
    """Encode static phenological features with a multilayer perceptron."""
...
class Model(nn.Module):
    """Multi-branch classification network that fuses spectral, spatial, SAR, and phenological features."""
