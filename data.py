"""Data loading, splitting, feature extraction, and augmentation for time-series samples."""
...
class TSLoader(object):
    """Load, split, augment, and package multi-source time-series samples for model training and evaluation."""

    def __init__(self, path, split, split_n, seed):
        """Read the input CSV file and initialize split ratios and random seed."""
    ...
    def get_phenology_data(self, data):
        """Extract static phenological features from the input sample table."""
        
        return data[:, -7:-2].astype("float32")

    def normalize(self, X, mean=None, std=None):
        """Standardize feature arrays using the provided or computed mean and standard deviation."""
    ...
    def add_noise(self, X, scale=0.01):
        """Apply Gaussian noise augmentation to input features."""

        noise = np.random.normal(0, scale, X.shape)
        return X + noise

    def smoothing(self, X, window_size=3):
        """Smooth a one-dimensional sequence with a moving-average window."""

        return np.convolve(X, np.ones(window_size) / window_size, mode='valid')

    def random_scaling(self, X, scale_range=(0.8, 1.2)):
        """Randomly scale input features within a specified range."""

        scale_factor = random.uniform(scale_range[0], scale_range[1])
        return X * scale_factor

    def random_time_flip(self, X):
        """Randomly reverse the temporal order of a time-series sample."""
    ...
    def noise_phenology(self, x_phe, std=0.03):
        """Add Gaussian noise to phenological features for data augmentation."""

        noise = np.random.normal(0, std, size=x_phe.shape)
        return x_phe + noise

    def augment(self, X_spectral, X_spatial, X_sar, X_phe, Y):
        """Generate augmented training samples using noise, temporal shift, scaling, and time flipping."""
    ...
    def get_data(self):
        """Split samples by class and extract spectral, spatial, SAR, and phenological feature tensors."""
        ...
        def extract_features(data):
            """Reshape tabular monthly features into spectral, spatial, SAR, and phenological tensors."""
    ...
    def return_data(self):
        """Convert extracted arrays into dataset lists used by PyTorch DataLoader."""
